...
import { Shield, LayoutDashboard, Building2, PackageCheck, LogOut, Key, AlertCircle } from 'lucide-react';
...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem('erp_token', data.data.token); // set for client calls
  ...
  // 1. Guard: If not authenticated as admin, show secure admin login screen
  if (!token) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-950 p-4">
        <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-8 shadow-2xl">
          ...
          <h1 className="mt-4 text-center text-xl font-bold text-white">Superadmin Control Center</h1>
          <p className="mt-1 text-center text-xs text-slate-400">
            Master multi-tenant platform credentials required
          </p>
          ...
          <form onSubmit={handleAdminLogin} className="mt-6 space-y-4">
            ...
            >
              {isSubmitting ? 'Verifying access...' : 'Access Admin Console'}
            </button>
          </form>
  ...
  // 2. Authenticated Admin Console Shell
  const adminNav = [
