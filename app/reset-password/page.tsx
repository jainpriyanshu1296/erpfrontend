...
function ResetPasswordForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try { await authApi.resetPassword({ token: params.get('token') || '', password }); setDone(true); window.setTimeout(() => router.push('/login'), 1200); } catch (e) { setError(e instanceof Error ? e.message : 'Unable to reset password'); }
  }
