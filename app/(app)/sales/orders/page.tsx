...
export default function Page() {
  ...
  const query = useQuery({
    queryKey: ['sales-orders', search, page, limit],
    queryFn: async () => {
      const params: Record<string, string> = { page: String(page), limit: String(limit) };
      if (search) params.search = search;
      const r = await recordsApi('/sales/orders').list(params);
      return { rows: (r.data || []) as SO[], total: Number(r.meta?.total || 0) };
    },
  });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 rounded-xl border bg-white p-3">
        ...
        <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search sales orders…" className="w-full outline-none text-sm" />
      </div>
