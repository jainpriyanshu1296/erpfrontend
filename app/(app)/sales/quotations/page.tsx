...
interface Quotation { id: string; quotation_number: string; customer_id: string; status: string; total_amount: number; valid_until: string; created_at: string; }
...
export default function Page() {
  ...
  const [lines, setLines] = useState([{ item_id: '', qty: '1', rate: '0', gst_rate: '18' }]);
  ...
  const query = useQuery({ queryKey: ['quotations'], queryFn: async () => { const r = await recordsApi('/sales/quotations').list({ limit: '100' }); return (r.data || []) as Quotation[]; } });

  const total = lines.reduce((s, l) => { const t = Number(l.qty) * Number(l.rate); return s + t + t * Number(l.gst_rate) / 100; }, 0);

  const createMutation = useMutation({
    mutationFn: () => recordsApi('/sales/quotations').create({ customer_id: customerId, valid_until: validUntil, total_amount: total, status: 'draft' }),
    onSuccess: () => { client.invalidateQueries({ queryKey: ['quotations'] }); showToast('Quotation created', 'success'); setOpen(false); setCustomerId(''); setValidUntil(''); setLines([{ item_id: '', qty: '1', rate: '0', gst_rate: '18' }]); },
    onError: (e: unknown) => showToast(e instanceof Error ? e.message : 'Failed', 'error'),
  });

  const convertMutation = useMutation({
    mutationFn: (id: string) => workflowApi.createOrderFromQuotation(id),
  ...
  const rows = (query.data || []).filter(r => !search || r.quotation_number?.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Prepare and send customer quotations.</p>
        </div>
        ...
        <div className="rounded-xl border bg-white p-5 shadow-sm space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block text-xs font-medium text-slate-700">Customer *
              <select className={`mt-1 ${inp}`} value={customerId} onChange={e => setCustomerId(e.target.value)} required>
          ...
          <div>
            ...
            {lines.map((l, i) => (
              <div key={i} className="grid items-end gap-3 mb-2 sm:grid-cols-5">
                <label className="block text-xs font-medium text-slate-700 sm:col-span-2">Item
                  <select className={`mt-1 ${inp}`} value={l.item_id} onChange={e => { const it = items.find(x => x.id === e.target.value); setLines(ls => ls.map((ll, j) => j === i ? { ...ll, item_id: e.target.value, rate: String(it?.standard_cost || ll.rate), gst_rate: String(it?.gst_rate || ll.gst_rate) } : ll)); }}>
                ...
                <label className="block text-xs font-medium text-slate-700">Qty
                  <input type="number" min="0.001" className={`mt-1 ${inp}`} value={l.qty} onChange={e => setLines(ls => ls.map((ll, j) => j === i ? { ...ll, qty: e.target.value } : ll))} />
                </label>
                ...
                <div className="flex items-end gap-2">
                  <label className="block flex-1 text-xs font-medium text-slate-700">GST %
            ...
            <div className="flex items-center justify-between mt-2">
              <button onClick={() => setLines(ls => [...ls, { item_id: '', qty: '1', rate: '0', gst_rate: '18' }])} className="rounded-lg border px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50"><Plus size={13} className="inline mr-1" />Add line</button>
          ...
          <div className="flex justify-end gap-2 pt-2 border-t">
            ...
            <button disabled={createMutation.isPending || !customerId} onClick={() => createMutation.mutate()} className="rounded-lg bg-indigo-600 px-5 py-2 text-sm font-semibold text-white disabled:opacity-50">
              ...
              <tbody>
                {rows.map(row => (
                  <tr key={row.id} className="border-b last:border-0 hover:bg-indigo-50/40">
                    ...
                    <td className="px-4 py-3 text-slate-600">{row.customer_id}</td>
                    ...
                    <td className="px-4 py-3">
                      {row.status === 'accepted' && (
                        <button onClick={() => convertMutation.mutate(row.id)} disabled={convertMutation.isPending} className="flex items-center gap-1 rounded bg-green-50 px-2.5 py-1 text-xs font-semibold text-green-700 hover:bg-green-100 disabled:opacity-50">
