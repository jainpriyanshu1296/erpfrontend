...
interface PO { id: string; po_number: string; vendor_id: string; status: string; total_amount: number; delivery_date: string; created_at: string; }
...
export default function Page() {
  ...
  const query = useQuery({ queryKey: ['purchase-orders'], queryFn: async () => { const r = await recordsApi('/purchase/orders').list({ limit: '100' }); return (r.data || []) as PO[]; } });

  const total = lines.reduce((s, l) => s + Number(l.quantity) * Number(l.rate), 0);

  const createMutation = useMutation({
    mutationFn: async () => {
      await workflowApi.createOrderFromRequisition({ vendor_id: vendorId, warehouse_id: warehouseId, delivery_date: deliveryDate, items: lines.map(l => ({ item_id: l.item_id, quantity: Number(l.quantity), rate: Number(l.rate) })) });
    },
    onSuccess: () => { client.invalidateQueries({ queryKey: ['purchase-orders'] }); showToast('PO created', 'success'); setOpen(false); setVendorId(''); setWarehouseId(''); setDeliveryDate(''); setLines([{ item_id: '', quantity: '1', rate: '0' }]); },
    onError: async () => {
      // Fallback: create PO directly
      try {
        await recordsApi('/purchase/orders').create({ vendor_id: vendorId, warehouse_id: warehouseId, delivery_date: deliveryDate, total_amount: total, status: 'draft' });
        client.invalidateQueries({ queryKey: ['purchase-orders'] });
        showToast('PO created', 'success'); setOpen(false);
      } catch (e: unknown) { showToast(e instanceof Error ? e.message : 'Failed', 'error'); }
    },
  });

  const approveMutation = useMutation({
    mutationFn: (id: string) => workflowApi.approvePurchaseOrder(id),
  ...
  const rows = (query.data || []).filter(r => !search || r.po_number?.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 rounded-xl border bg-white p-3">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search purchase orders…" className="w-full outline-none text-sm" />
      </div>
        ...
        <div className="rounded-xl border bg-white p-5 shadow-sm space-y-4">
          ...
          <div>
            ...
            {lines.map((l, i) => (
              <div key={i} className="grid items-end gap-3 mb-2 sm:grid-cols-4">
                <label className="block text-xs font-medium text-slate-700 sm:col-span-2">Item
                  <select className={`mt-1 ${inp}`} value={l.item_id} onChange={e => { const it = items.find(x => x.id === e.target.value); setLines(ls => ls.map((ll, j) => j === i ? { ...ll, item_id: e.target.value, rate: String(it?.standard_cost || ll.rate) } : ll)); }}>
            ...
            <div className="flex items-center justify-between mt-2">
              <button onClick={() => setLines(ls => [...ls, { item_id: '', quantity: '1', rate: '0' }])} className="rounded-lg border px-3 py-1.5 text-xs font-medium text-slate-600"><Plus size={13} className="inline mr-1" />Add line</button>
          ...
          <div className="flex justify-end gap-2 pt-2 border-t">
            ...
            <button disabled={createMutation.isPending || !vendorId} onClick={() => createMutation.mutate()} className="rounded-lg bg-indigo-600 px-5 py-2 text-sm font-semibold text-white disabled:opacity-50">
              ...
              <tbody>
                {rows.map(row => (
                  <tr key={row.id} className="border-b last:border-0 hover:bg-indigo-50/40">
                    <td className="px-4 py-3 font-mono font-semibold text-indigo-700">{row.po_number}</td>
                    <td className="px-4 py-3 text-slate-600">{row.vendor_id}</td>
                    ...
                    <td className="px-4 py-3">
                      {row.status === 'draft' && (
                        <button onClick={() => approveMutation.mutate(row.id)} disabled={approveMutation.isPending} className="flex items-center gap-1 rounded bg-green-50 px-2.5 py-1 text-xs font-semibold text-green-700 hover:bg-green-100 disabled:opacity-50">
